...
class Moves():
    def validate(self, turn, piece_to_move, piece_index, x, y, black, white, move_array=None, check=False, safe_moves=[]):
        if [x, y] == [10, 4] or [x, y] == [12, 4]:
            return False
        if turn == "black":
            if self.validate_player(turn, piece_to_move, piece_index, x, y, black, white, move_array, check, safe_moves):
                return False
        elif turn == "white":
            if self.validate_player(turn, piece_to_move, piece_index, x, y, white, black, move_array, check, safe_moves):
                return False
        return True

    def validate_player(self, turn, piece_to_move, piece_index, x, y, player, other_player, move_array, check, safe_moves):
        if not move_array:
            move_array = self.get_move_array(turn ,piece_to_move, piece_index, x, y,player, other_player, check, safe_moves)
        try:
    ...
    def get_rook_moves(self, player, other_player, piece, piece_index, move_array, all_player_coords, check, safe_moves):
        ...
        if "PR " == piece:
            for val in [-1, 1]:
                ...
                if self.__inside_the_board(new_coords1) and new_coords1 not in all_player_coords and ((not check) or (new_coords1 in safe_moves[(piece, piece_index)])):
                    move_array.append(new_coords1)
                if self.__inside_the_board(new_coords2) and new_coords2 not in all_player_coords and ((not check) or (new_coords2 in safe_moves[(piece, piece_index)])):
